...
public class ProvenanceTracker {
	private static String driverName = "org.apache.hive.jdbc.HiveDriver";
	   ...
	   public static boolean schemaExists(String arg1) throws ClassNotFoundException, SQLException {
	      ...
	      if(set.next()){con.close();return true;}
	...
	public static void main(String[] args) throws IOException, InterruptedException, ClassNotFoundException, SQLException {
	   ...
	   String user=null,path=null,timestamp=null,type=null;
	   if(args.length>3){
		   System.out.println("Error in input");
	       System.exit(1);
	   }
	   for(int i=0;i<args.length;i++){
		   if(!(args[i].contains(" -u")||args[i].contains(" -t")||args[i].contains(" -d")||args[i].contains(" -type"))){
			   System.out.println("Input error");
			   System.exit(1);
		   }
		   else if(args[i].contains(" -u")){
			   String delim[]=args[i].split("\\ -u");
			   user=delim[0];
		   }
		   else if(args[i].contains(" -t")){
			   String delim[]=args[i].split("\\ -t");
			   timestamp=delim[0];
		   }
		   else if(args[i].contains(" -d")){
			   String delim[]=args[i].split("\\ -d");
			   path=delim[0];
		   }
		   else if(args[i].contains(" -type")){
			   String delim[]=args[i].split("\\ -type");
			   type=delim[0];
		   }
	   }
	   	   getProvenanceInformation(user, timestamp, path, type);
	}
